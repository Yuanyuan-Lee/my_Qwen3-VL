...
def process_single_episode(episode_idx, hdf5_dir, instructions_dir, save_dir, task_name, task_level, segment_size, total_segments, total_frames, epsilons=None):
    try:
        ...
        for sample_idx in range(sample_num):
            ...
            # 如果没有外部传入 epsilons，使用默认 0.002
            if epsilons is None:
                epsilons_local = 0.002
            else:
            ...
            if len(seen_list) > 0:
                seen_instr = np.random.choice(seen_list)
                episode_info_seen = {
                    "image": image_paths,
                ...
                logging.warning(f"No 'seen' instructions in {instruction_file}, skipping seen sample.")

            # 采样 unseen 指令
            unseen_list = instructions.get('unseen', [])
            if len(unseen_list) > 0:
                unseen_instr = np.random.choice(unseen_list)
                episode_info_unseen = {
                    "image": image_paths,
                ...
                logging.warning(f"No 'unseen' instructions in {instruction_file}, skipping unseen sample.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epsilons", type=str, default="",
                        help="Comma-separated epsilons. Length 1, 3 (pos,rot,grip) or 14 per-dim. Default empty -> use 0.002")
    args = parser.parse_args()

    # 解析 epsilons 参数（支持长度 1, 3 或 14；空字符串 -> None，后续使用默认 0.002）
    if args.epsilons:
