...
def process_single_episode(episode_idx, hdf5_dir, instructions_dir, save_dir, task_name, task_level, segment_size, total_segments, total_frames):
    try:
        ...
        for sample_idx in range(sample_num):
            ...
            def bucketize(delta, pos_eps=0.002, rot_eps=0.003, grip_eps=0.005):
            ...
            # 从 instructions 中分别随机采样 seen 与 unseen（包含索引0）
            ...
            # helper: 构建 human conversation 文本
            def build_human_conv(instr_text):
                return {
                    ...
                    "value": f"You are a Aloha-AgileX robot using end-effector control. The instruction is \"{instr_text}\".<image>\nThis is the left camera image of the current frame.\n<image>\nThis is the head camera image of the current frame.\n<image>\nThis is the right camera image of the current frame.\n"
                             ...
                             f"We define a trajectory label as follows: every {segment_size} frames, take the endpose differences. Each component is mapped to 0 if below the threshold, 1 if within [-threshold, threshold], and 2 if above the threshold. The thresholds for xyz are 0.002, for rpy are 0.003, and for gripper are 0.005.\n"
                             f"Please predict the next {total_segments} trajectory labels based on the images, instruction and the previous three frames' endposes. Please output them in order using \"_\" to connect different trajectory labels like \"11010211111111_10000211111111_…\""
                }
