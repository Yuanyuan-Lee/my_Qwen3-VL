...
# 运行脚本
hdf5_dir = '/share/project/liyuanyuan/code/RoboTwin/data/stack_bowls_two/aloha-agilex_clean_50/data'
instructions_dir = '/share/project/liyuanyuan/code/RoboTwin/data/stack_bowls_two/aloha-agilex_clean_50/instructions'
save_dir = '/share/project/liyuanyuan/code/Qwen3-VL/qwen3_vl_data/'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
task_name = 'stack_bowls_two'
task_level = 'clean'

# 自动检测 episode 文件数量（episode0.hdf5 ... episodeN.hdf5）
hdf5_files = sorted(
    [f for f in os.listdir(hdf5_dir) if f.startswith("episode") and f.endswith(".hdf5")]
)
if len(hdf5_files) == 0:
    raise RuntimeError(f"No episode .hdf5 files found in {hdf5_dir}")
episode_count = len(hdf5_files)
print(f"Detected {episode_count} episode files, will process all of them.")
